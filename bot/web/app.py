...
def render(request: Request, name: str, status_code: int = 200, **context) -> Response:
    ...
    context = {"csrf": csrf, "flash": request.session.pop("flash", None), "path": request.url.path, **context}
...
def create_app(bot: Bot, db: Database, sheets: Sheets | None, broadcaster: Broadcaster) -> FastAPI:
    ...
    async def login(request: Request, username: str = Form(""), password: str = Form(""), next: str = Form("/")):
        ...
        valid_password = secrets.compare_digest(password.encode(), settings.admin_password.encode())
        if not (valid_user and valid_password):
            failures.append(now)
            log.warning("Web panelga noto'g'ri kirish urinishi (ip=%s)", ip)
    ...
    @panel.post("/sync", dependencies=[Depends(verify_csrf)])
    ...
    async def application_status(request: Request, user_id: int, status: str = Form(...), notify: bool = Form(False)):
    ...
    @panel.post("/broadcast", dependencies=[Depends(verify_csrf)])
    async def broadcast_send(
        request: Request, audience: str = Form("all"), text: str = Form(""), action: str = Form("start")
    ):
        text = text.strip()
        error = None
        ...
        job = await broadcaster.start(audience, send, started_by="web")
        flash(request, f"Tarqatma boshlandi: {job.total} ta foydalanuvchi.")
        return RedirectResponse("/broadcast", status_code=303)

    @panel.post("/broadcast/stop", dependencies=[Depends(verify_csrf)])
